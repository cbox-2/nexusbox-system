      ...
      if (d.success && d.messages) {
        ...
        if (tbody) {
          ...
          d.messages.forEach(m => {
            const row = tbody.insertRow();
            row.innerHTML = `
              <td><input type="checkbox" name="mid" value="${m._id}"></td>
              <td>${m.content || ''}</td>
              <td>${m.author?.username || 'Guest'}</td>
              <td>${new Date(m.createdAt).toLocaleString()}</td>
              <td>
                <input type="button" value="Archive" onclick="archiveMessage('${m._id}')" class="button">
                <input type="button" value="Delete" onclick="deleteMessage('${m._id}')" class="Danger">
              </td>
            `;
          });
        }
...
window.archiveMessage = function(id) {
  fetch(API + '/api/messages/' + id + '/archive', {
    method: 'PUT',
    headers: { 'Authorization': 'Bearer ' + TOKEN, 'Content-Type': 'application/json' },
...
// ===== STICKY MESSAGES (messages-sticky.html) =====
function loadStickyPage() {
  if (!currentBoxId) return;
  fetch(API + '/api/boxes/' + currentBoxId + '/messages?sticky=true', { headers: { 'Authorization': 'Bearer ' + TOKEN } })
    .then(r => r.json())
    .then(d => {
        ...
        if (tbody) {
          ...
          d.messages.forEach(m => {
            ...
            row.innerHTML = `
              ...
              <td>${new Date(m.createdAt).toLocaleString()}</td>
              <td>
                <input type="button" value="Edit" onclick="editSticky('${m._id}', this)">
                <input type="button" value="Delete" onclick="deleteSticky('${m._id}')" class="Danger">
              </td>
            `;
          });
        }
...
window.editSticky = function(id, btn) {
  const newContent = prompt('Edit message:', btn.closest('tr').cells[0].textContent);
  if (newContent !== null) {
    fetch(API + '/api/messages/' + id + '/sticky', {
      method: 'PUT',
      headers: { 'Authorization': 'Bearer ' + TOKEN, 'Content-Type': 'application/json' },
...
window.deleteSticky = function(id) {
  if (!confirm('Delete this sticky message?')) return;
  fetch(API + '/api/messages/' + id + '/sticky', {
    method: 'DELETE',
...
// ===== ARCHIVE PAGE (messages-archive.html) =====
function loadArchivePage() {
  if (!currentBoxId) return;
  fetch(API + '/api/boxes/' + currentBoxId + '/messages/archived', { headers: { 'Authorization': 'Bearer ' + TOKEN } })
    .then(r => r.json())
    .then(d => {
      if (d.success && d.messages) {
        const tbody = document.querySelector('table tbody');
        if (tbody) {
          ...
          d.messages.forEach(m => {
            ...
            row.innerHTML = `
              ...
              <td>${new Date(m.createdAt).toLocaleString()}</td>
              <td>
                <input type="button" value="Restore" onclick="restoreMessage('${m._id}')">
                <input type="button" value="Delete" onclick="deleteMessage('${m._id}')" class="Danger">
              </td>
            `;
          });
...
window.restoreMessage = function(id) {
  fetch(API + '/api/messages/' + id + '/archive', {
    method: 'PUT',
    headers: { 'Authorization': 'Bearer ' + TOKEN, 'Content-Type': 'application/json' },
...
// ===== USER SETTINGS (users.html - fuseropt) =====
function loadUserSettings() {
  if (!currentBoxId) return;
  fetch(API + '/api/boxes/' + currentBoxId + '/user-settings', { headers: { 'Authorization': 'Bearer ' + TOKEN } })
    .then(r => r.json())
    .then(d => {
      if (d.success && d.userSettings) {
        const f = document.forms['fuseropt'];
        if (f) {
          if (f.regonly) f.regonly.checked = d.userSettings.regonly || false;
          if (f.selfreg) f.selfreg.checked = d.userSettings.selfreg !== false;
          if (f.autoprotect) f.autoprotect.checked = d.userSettings.autoprotect || false;
          if (f.autouserdel) f.autouserdel.checked = d.userSettings.autouserdel || false;
          if (f.lastpostdel) f.lastpostdel.checked = d.userSettings.lastpostdel || false;
        }
      }
    });
}

// ===== BAN POLICY (users-banned.html - fcban) =====
function loadBanPolicy() {
  if (!currentBoxId) return;
  fetch(API + '/api/boxes/' + currentBoxId + '/ban-policy', { headers: { 'Authorization': 'Bearer ' + TOKEN } })
    .then(r => r.json())
    .then(d => {
      if (d.success && d.banPolicy) {
        const f = document.forms['fcban'];
        if (f) {
          if (f.policy) f.policy.value = d.banPolicy.policy || '';
          if (f.country) f.country.value = d.banPolicy.country || '';
        }
      }
    });
}

// ===== CHANNELS UPDATE (messages-channels.html) =====
        ...
        if (tbody) {
          ...
          d.channels.forEach(c => {
            ...
            row.innerHTML = `
              ...
              <td>${c.description || '-'}</td>
              <td>
                <input type="button" value="Edit" onclick="editChannel('${c._id}', this)">
                <input type="button" value="Delete" onclick="deleteChannel('${c._id}')" class="Danger">
              </td>
            `;
          });
        }
...
window.editChannel = function(id, btn) {
  const newName = prompt('Edit channel name:', btn.closest('tr').cells[0].textContent);
  if (newName !== null) {
...
// ===== PUBLISH ADVANCED (publish.html - fsnippet2) =====
function loadPublishAdvanced() {
  if (!currentBoxId) return;
  fetch(API + '/api/boxes/' + currentBoxId + '/publish-advanced', { headers: { 'Authorization': 'Bearer ' + TOKEN } })
    .then(r => r.json())
    .then(d => {
      if (d.success && d.publishAdvanced) {
        const f = document.forms['fsnippet2'];
        if (f) {
          if (f.cpy) f.cpy.value = d.publishAdvanced.cpy || '';
          if (f.variation) f.variation.value = d.publishAdvanced.variation || '';
          if (f.sitetype) f.sitetype.value = d.publishAdvanced.sitetype || '';
        }
      }
    });
}
...
// ===== INTERCEPT NEW FORMS =====
function interceptNewForms() {
  // fuseropt (users.html)
  const useroptForm = document.forms['fuseropt'];
  if (useroptForm) {
    useroptForm.addEventListener('submit', function(e) {
      e.preventDefault();
      if (!currentBoxId) return alert('No box selected');
      fetch(API + '/api/boxes/' + currentBoxId + '/user-settings', {
        method: 'PUT',
        headers: { 'Authorization': 'Bearer ' + TOKEN, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          regonly: this.regonly?.checked || false,
          selfreg: this.selfreg?.checked !== false,
          autoprotect: this.autoprotect?.checked || false,
          autouserdel: this.autouserdel?.checked || false,
          lastpostdel: this.lastpostdel?.checked || false
        })
      }).then(r => r.json()).then(d => alert(d.success ? 'User settings saved!' : 'Error')).catch(() => alert('Error'));
    });
  }

  // fcban (users-banned.html)
  const fcbanForm = document.forms['fcban'];
  if (fcbanForm) {
    fcbanForm.addEventListener('submit', function(e) {
      e.preventDefault();
      if (!currentBoxId) return alert('No box selected');
      fetch(API + '/api/boxes/' + currentBoxId + '/ban-policy', {
        method: 'PUT',
        headers: { 'Authorization': 'Bearer ' + TOKEN, 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      }).then(r => r.json()).then(d => alert(d.success ? 'Ban policy saved!' : 'Error')).catch(() => alert('Error'));
    });
  }

  // fsnippet2 (publish.html)
  const fsnippet2Form = document.forms['fsnippet2'];
  if (fsnippet2Form) {
    fsnippet2Form.addEventListener('submit', function(e) {
      e.preventDefault();
      if (!currentBoxId) return alert('No box selected');
      fetch(API + '/api/boxes/' + currentBoxId + '/publish-advanced', {
        method: 'PUT',
        headers: { 'Authorization': 'Bearer ' + TOKEN, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          cpy: this.cpy?.value || '',
          variation: this.variation?.value || '',
          sitetype: this.sitetype?.value || ''
        })
      }).then(r => r.json()).then(d => alert(d.success ? 'Publish settings saved!' : 'Error')).catch(() => alert('Error'));
    });
  }

  // fpostsarc (messages-archive.html - search)
  const archiveSearchForm = document.forms['fpostsarc'];
  if (archiveSearchForm) {
    archiveSearchForm.addEventListener('submit', function(e) {
      e.preventDefault();
      if (!currentBoxId) return alert('No box selected');
      const params = new URLSearchParams({
        from: `${this.yy1?.value}-${this.mm1?.value}-${this.dd1?.value}`,
        to: `${this.yy2?.value}-${this.mm2?.value}-${this.dd2?.value}`
      });
      fetch(API + '/api/boxes/' + currentBoxId + '/messages/archived?' + params, { headers: { 'Authorization': 'Bearer ' + TOKEN } })
        .then(r => r.json())
        .then(d => {
          if (d.success) {
            alert('Found ' + d.messages.length + ' archived messages');
          }
        });
    });
  }

  // fquickdel (messages.html - quick delete)
  const quickdelForm = document.forms['fquickdel'];
  if (quickdelForm) {
    quickdelForm.addEventListener('submit', function(e) {
      e.preventDefault();
      if (!currentBoxId) return alert('No box selected');
      const selected = Array.from(document.querySelectorAll('input[name="mid"]:checked')).map(cb => cb.value);
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  if (path.includes('messages.html') && !path.includes('messages-')) loadMessagesPage();
  if (path.includes('messages-sticky.html')) loadStickyPage();
  if (path.includes('messages-archive.html')) loadArchivePage();
  if (path.includes('messages-channels.html')) loadChannelsPage();
  if (path.includes('users.html') && !path.includes('users-')) loadUserSettings();
  if (path.includes('users-banned.html')) loadBanPolicy();
  if (path.includes('publish.html')) loadPublishAdvanced();
});
