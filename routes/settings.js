...
router.post('/publish/generate-tag', authMiddleware, async (req, res) => {
  try { const tag = Math.round(Math.random() * 1838265624).toString(35).replace('u', 'z').replace('i', '5').replace('o', 'w'); res.json({ success: true, tag }); }
  catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
router.put('/layout', authMiddleware, async (req, res) => {
  try {
    const data = req.body;
    let settings = await LayoutSettings.findOne({ user: req.userId });
    Object.keys(data).forEach(key => { if (settings[key] !== undefined) settings[key] = data[key]; });
    settings.updatedAt = new Date();
    await settings.save();
    res.json({ success: true, settings });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
